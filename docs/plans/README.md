...
| M6 | The steady-state ladder: where the grid sits before anything moves — line resistance, generator voltage setpoints and reactive limits, a declared slack; a sparse DC solve and an AC Newton solve on `NetworkModel`; two oracles; the editor folded in | **In progress — all planned steps done (0–7); the milestone close is the user's call.** **Step 7 (2026-09-23) built the network-free cheapest dispatch**: cost and minimum on `Machine` (NaN = not given, refused by name), `JuMP` + `HiGHS` as a **package extension** (core's manifest stays at 187 packages without them), and a dispatch handed to the AC flow with what the network adds reported in two parts — losses, and the shift in what voltage-dependent loads draw (+109.26 $/h on 5216.03 for case9-without-line-charging with constant-power loads; **−318 $/h with the repo's default constant-impedance loads**, found by review after the first commit, when the report still called the sum "the losses' cost"). The cost data is MATPOWER `case9`'s — **published but unsourced**, chosen by the user over the textbook, so the plan's printed-answer check does not exist; every oracle computes in MW from the file's rows typed into the test, and an `S_base`-invariance check stands in — both catch the one-factor-short per-unit mutation. Step 6's 2.0e-4 MW solver gap turned out to be HiGHS's QP regularization acting on a problem posed in MW (1.5e-9 MW posed in pu, 2.8e-14 with it off); the plan's "exactly zero on a lossless network" was false (rounding, 2e-16); "load above every maximum" is unreachable by construction; and the first "maximum ignored" mutation went red only because it broke the extension's compile. Leaves **3602 core / 1140 reference / 446 UI** on re-resolved manifests, M5's criterion values bit-identical. Before it: steps 0–6 done (BOTH oracles; the editor and the scenario file folded in; **the optimisation gate decided 2026-09-23 — opened NARROW**). **Step 6 re-measured all three suites on re-resolved manifests (3312 core / 1140 reference / 443 UI) and found the gate's two failures are different in kind**: no cost data exists in the repo (fixable by importing a cited published case), and `PowerModels`, the external optimal-power-flow checker, does not resolve against our stack at any version (not fixable by us). So the **network OPF is OWED on criterion 4**, and step 7 builds only the **network-free cheapest dispatch** (`JuMP` + `HiGHS`, +15 packages, nothing moved), checked by a closed form, a printed textbook answer and an exhaustive search — none of them the solver grading itself (`m6-context.md` D16). Entered at `181fe4e` with 2835 core / 382 UI / 986 reference; **2996 core** at step 2's close, **3175 core** at step 3's, **3284 core** at oracle A's, and **1140 reference** at oracle B's (986 unchanged + 154 new, core and UI untouched by that step). **Step 4's oracle B is the round where the ORACLE was the thing measured**: `PowerFlows` stores its admittance matrix in `ComplexF32` (`PowerNetworkMatrices/src/definitions.jl` line 1), so its Newton converges honestly to 4.4e-16 — on a rounded network — while its answers leave 2.0e-7 to 3.8e-8 pu of mismatch in an independently built double-precision `Y` where ours leave 4.4e-16 to 2.9e-14. That is M4 D7's "the oracle is a floor, not a ceiling" arriving a second time by a different mechanism, and it forced the band to be rebuilt: each side's own convergence is four orders too small, and two attempts to *predict* their error (a Float32 twin; a first-order per-branch sum) were both measured and thrown away, because their rounded diagonal breaks the shunt-free identity `Y_vv = −Σ Y_vu` that our model holds by construction — a perturbation our model cannot express, and therefore not one a factor can cover (D15). The shipped band is a statement about their storage with **no factor**, and the check that actually carries the step needs no band at all. Oracle B also closed the two things oracle A structurally could not reach — a **lossy** branch (`Branch.R`'s only numerical check anywhere) and a **binding reactive limit** — and corrected D11's prediction that its fixtures would need constant-power loads: their `StandardLoad` holds the full ZIP split and its law is exactly ours, which matters because `Load`'s default share is constant *impedance*. Step 3 shipped the AC Newton solve — our residual equations, `NonlinearSolve`'s solver, a sparse complex `Y` checked structurally, reactive-limit switching, and the `|V|` band inherited by **splitting** `_check_power_flow` into three named checks rather than copying the constant (D10). Its six sabotages found **two checks reading their answer from the source they were checking**: the losses identity computed the load draw through the very function a sabotage had broken, and the band test asserted "outside" without asserting which *side*, so it passed against a sign flip that pushed the voltage out of the other end. Both were rewritten and the mutation coverage went from 1 and 3 testsets to 4 each. Unlike step 2, **no single check carries this step** — four of six is the best any one manages, and it takes a lossless fixture together with a lossy one to cover the set. The rate control's band ([7.5, 8.5], measured 8.020/8.005/8.001) was written before the gap was seen, and the fixture was then *proved* load-bearing: the same check with a scaled reactive load gives 4.2/4.1/4.1, non-overlapping. **Step 5 folded the editor and the file back in.** A scenario file with no `slack` is now REFUSED rather than defaulted (`m6-context.md` D8: every other absence has a physical reading — a branch with no `R` was a lossless branch — and a reference bus has none, so inventing one records a dispatch choice as though the file had said so), with a message naming **every** bus rather than the machine-carrying subset, because teaching `ac_powerflow`'s voltage-source rule in the *reader's* error text would make the reader a second validator (step 5, F1). The editor's panel grew the three machine fields a power flow reads plus a branch's `R`; the reference bus is drawn on the map — labelled `slack (derived)` when nobody declared one, so the choice a save would otherwise write silently is visible — and chosen from a **make slack** / **release slack** button; and **solve** puts `ac_powerflow`'s answer on the drawing, cleared by any edit, with both refusal kinds caught (the voltage band is an `ErrorException`, and it is the one a hand-drawn scenario meets first). Four renders changed the design three times: the twelve-field panel ran off the bottom of the window exactly as nine had at the editor's own build, and the fix had to be the **shape** (two fields per row) rather than the size, since shrinking bought 84 px and one more field; `arrows2d!` cannot be constructed empty, which is this overlay's resting state, and the *precompile workload* is what caught it; and the obvious demo fixture was wrong for the fourth time in this repo — every bus of `three_machine_ring()` solves to exactly 1.000 pu with the slack picking up exactly its schedule, so the figure uses `load_bus_system()`, where the load bus sags to 0.955 and the slack picks up 60.42 MW against 70.0. Entry counts were **measured on a stashed tree** rather than carried forward: 3284 core / 382 UI at `b758027`, leaving 3312 / 443. Step 1 added the fields under an invariant that let no existing number move — checked twice over, the whole suite green AND M5's 169 recorded criterion values bit-identical by MD5, captured at HEAD before the first edit because those numbers stop existing the moment the tree changes. Step 2 shipped the sparse DC solve, **the first matrix this repo assembles itself**, and its five implementation sabotages found that the check the plan called the step's real discriminator — superposition — is blind to all five, because every one of them yields a different but still LINEAR map (step 2, F6). What carries the step is a three-bus split on unequal reactances, where the answer is fixed by algebra and there is no symmetry to hide behind. The step-0 measurement was taken **before** any plan prose committed to it: `PowerSystems` 5.12.3 + `PowerFlows` 0.25.2 resolve on top of our exact stack (258 packages against 184), move **nothing** of ours (SciMLBase 3.53.1, OrdinaryDiffEq 7.8.1, NetworkDynamics 1.3.0 all unchanged) and are *usable* — a two-bus case solved by both their AC and DC entry points — so the failure that killed PSID in M4 does not recur here. **`SPEC.md` §9 item 5's "introduce `PowerSystems.jl` as canonical model" is therefore refused with the constraint lifted** rather than blocked: the package is adopted in `reference/` as the second oracle and `NetworkModel` stays canonical (D1). The equations are ours and the solver is `NonlinearSolve`'s, which is `SPEC.md` §8 read precisely rather than around (D2) — and both it and `SparseArrays` are already in the tree transitively, so they cost zero new packages. The existing `find_fixpoint` steady state is **not** a power flow and cannot be generalised into one (D5, the unknowns and the givens swap places); both survive, and step 4's flat run is what makes them agree without either being declared correct. Bus roles are derived from one declared slack plus what is attached, never stored (D3); `R` defaults to 0.0 so step 1's gate is the invariant that **no existing number moves**, with a mutation to prove the gate is not vacuous (D4); the optimisation rung is a **gate with four written criteria**, not a commitment (D7). | `m6-*.md` (no pre-study — the physics is textbook, the unknown is repo integration) |
